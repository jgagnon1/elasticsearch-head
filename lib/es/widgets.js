...
//		path: "",					// (required) path to request
	...
	es.ui.ClusterOverview = es.ui.Page.extend({
		...
		_cluster_template: function(cluster, indices) {
			...
			return { tag: "DIV", cls: "", children: [{ tag: "DIV", cls: "clusterOverview-cluster-divheader", children: [{ tag: "TABLE", cls: "clusterOverview-cluster clusterOverview-cluster-tableheader", children: [{ tag: "THEAD", child: { tag: "TR", children: indices.map(this._indexHeader_template, this) }  }] }]  },
			{ tag: "DIV", cls: "clusterOverview-cluster-details", children: [{ tag: "TABLE", cls: "clusterOverview-cluster clusterOverview-cluster-header", children: [cluster.aliases.length && { tag: "TBODY", children: cluster.aliases.map(this._alias_template, this) },{ tag: "TBODY", children: cluster.nodes.sort(nodeNameCmp).map(this._node_template, this)}] }] }] };
